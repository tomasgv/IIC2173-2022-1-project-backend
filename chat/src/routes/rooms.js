...
async function authorizeRoomAdmin(ctx, next) {
  ...
  if (
    room.entity_owner === ctx.state.tokendata.entityUUID &&
    ...
    if (
      roomPermission.some((element) => element.permissions.indexOf('a') > -1)
    ) {
      authorized = true;
...
async function authorizeRoom(ctx, next, permission) {
  ...
  if (
    ...
    if (
      roomPermission.some(
        (element) => element.permissions.indexOf(permission) > -1
      )
    ) {
      authorized = true;
      ctx.request.room_permission = roomPermission;
